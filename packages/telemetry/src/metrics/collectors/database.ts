...
export function createDatabaseMetricsCollector(options: DatabaseMetricsOptions) {
  ...
  const queryCount = metrics.counter('database_queries_total', 'Total number of database queries');
...
export function createPrismaMetricsMiddleware() {
  ...
  return async (params: any, next: any) => {
    ...
    try {
      ...
      collector.recordQuery(params.action, params.model || 'unknown', duration);

      return result;
    } catch (error: any) {
      const duration = Date.now() - start;

      collector.recordQuery(params.action, params.model || 'unknown', duration, error);
