...
export function createHttpMetricsCollector(options: HttpMetricsOptions = {}) {
  ...
  const requestCount = metrics.counter('http_requests_total', 'Total number of HTTP requests');
  ...
  return {
    ...
    onResponse(request: FastifyRequest, reply: FastifyReply) {
      ...
      if (options.includePath) {
        const path = options.normalizePath ? options.normalizePath(request.url) : request.url;
        tags.path = path;
      }
